<template>
  <div>
    <el-alert
      title="温馨提示"
      type="success"
      description="两个演示，简单的连线和优化链接端点的场景"
    >
    </el-alert>
    <div id="twoNode"></div>
  </div>
...
export default {
  ...
  mounted() {
    ...
    const graph = new Graph({
      container: document.getElementById('twoNode'),
      width: 1000,
      height: 1000,
    ...
    let map = {};
    map.nodes = dagMap.nodes.slice(0, 2);
    map.edges = dagMap.edges.slice(0, 1);
    graph.fromJSON(map);

    // 4. 设置节点状态
    this.nodeStatusList.slice(0, 2).forEach((item) => {
